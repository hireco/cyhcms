...
elseif(isset($_REQUEST['infor_class'])) { 
 ...
 elseif(isset($_REQUEST['picture'])) 
   ...
   exit();
    ...
    $query="select * from ".$table_suffix."article where  1 and keywords like '%{$keywords}%' order by  {$turn} desc";
    $_REQUEST['infor_class']="article";
   }
    ...
    <td width="120"><div align="center">
      <select name='channelid' style='width:110px' onchange="location=this.value;">
         <?php
			...
			?><option value="?infor_class=<?=$row->class_name?>&form_id=<?=$_REQUEST['form_id']?>" <?php if($_REQUEST['infor_class']==$row->class_name) { $infor_class_name=$row->chinese_name; echo "selected"; }?>><?=$row->chinese_name?></option><?php } ?>
      </select>
    </div></td>
  </tr>
</table>
  ...
  <tr>
    <td><table border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td>&nbsp;</td>
        <td><input name="edit_arc" type="button" class="inputbut" value="写入ID" onClick="ReturnValue('<?=$_REQUEST['form_id']?>');"></td>
        <td>&nbsp;</td>
...
<script>
function ReturnValue(obj_id)
{   var form_obj=window.opener.document.getElementById(obj_id);
    var old_str=form_obj.value;
