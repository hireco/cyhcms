...
{ echo "<script>alert(\"请选择该栏目的上级栏目!\"); history.go(-1);</script>"; exit; } 
$result=mysql_query("select * from  ".$table_suffix."infor  where id={$_REQUEST['class_id']}");
if(!mysql_num_rows($result)) 
{ echo "<script>alert(\"该栏目的上级栏目不存在!\"); history.go(-1);</script>"; exit; } 
$row=mysql_fetch_object($result);
if($row->class_attribute<'3') 
{ echo "<script>alert(\"该栏目不可以建下级栏目!\"); history.go(-1);</script>"; exit; } 
?>
<html>
    ...
    <td><table width="100%"  border="0" cellspacing="0" cellpadding="5">
        <tr>
          <td bgcolor="#FFFFFF"><table width="100%"  border="0" cellspacing="0" cellpadding="0">
            <tr>
              <td><form name="form1" method="post" action="infor_sub_add_submit.php" enctype="multipart/form-data">
                <table width="100%"  border="0" cellspacing="0" cellpadding="2">
                  <tr bgcolor="#F4D8AC">
                    <td colspan="2"><table border="0" align="left" cellpadding="0" cellspacing="0">
                      <tr>
                        <td width="10">&nbsp;</td>
                        <td>频道管理&gt;&gt;<a href="infor.php?infor_class=<?=$_REQUEST['infor_class']?>"><?=$_REQUEST['chinese_name']?></a>&gt;&gt;增加[<a href="infor_detail.php#<?=$_REQUEST['class_id']?>"><?=$_REQUEST['class_name']?></a>]的下级栏目</td>
                      </tr>
                    </table></td>
                    </tr>
                  <tr>
                    <td width="100">栏目名称：                      </td>
                    <td width="1096"><input name="class_name" type="text" id="class_name">
                      <input name="infor_class" type="hidden" value="<?=$_REQUEST['infor_class']?>">
                      <input name="upper_class_id" type="hidden" value="<?=$_REQUEST['class_id']?>">
                      <input name="class_level" type="hidden" value="<?php echo 1+$_REQUEST['class_level'];?>">
                      选择模版
                  ...
                  <tr>
                    ...
                    <td valign="top"><textarea name="keywords" cols="50" rows="4" id="keywords"></textarea></td>
                  </tr>
                  <tr>
                    <td>栏目属性：</td>
