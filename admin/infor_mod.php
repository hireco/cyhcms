...
{ echo "<script>alert(\"请选择栏目!\"); location.replace(\"infor.php?infor_class=$infor_class\");</script>"; exit; } 
$query="select * from ".$table_suffix."infor where id={$_REQUEST['class_id']} and infor_class='$infor_class'";
$result=mysql_query($query);
if(!$row=mysql_fetch_object($result))
{ echo "<script>alert(\"栏目不存在!\"); location.replace(\"infor.php?infor_class=$infor_class\");</script>"; exit; }
?>
<html>
                ...
                <table width="100%"  border="0" cellspacing="0" cellpadding="2">
                  <tr bgcolor="#F4D8AC">
                    <td colspan="2"><table border="0" align="left" cellpadding="0" cellspacing="0">
                      <tr>
                        <td width="10">&nbsp;</td>
                        <td>频道管理&gt;&gt;<a href="infor.php?infor_class=<?=$_REQUEST['infor_class']?>"><?=$_REQUEST['chinese_name']?></a>&gt;&gt;修改[<a href="infor_detail.php#<?=$_REQUEST['class_id']?>"><?=$_REQUEST['class_name']?></a>]栏目</td>
                      </tr>
                    </table></td>
                    </tr>
                  <tr>
                    <td width="100">所属上级：</td>
                    <td><select name="upper_class">
                      <option value="0 -1">根栏目</option>
					  <?php 
                  ...
                  <tr>
                    ...
                    <td><select name="top" id="top">
                      <option value="1" <?php if($row->top>="1") echo "selected"; ?>>置顶</option>
                      <option value="0" <?php if($row->top=="0") echo "selected"; ?>>不置顶</option>
                    </select>
                      置顶序号
...
*填写0,1,2等数字,不填则默认为前面选取值</td>
                  ...
                  <tr>
                    ...
                    <td><select name="top_navi" id="top_navi">
                      <option value="1" <?php if($row->top_navi=="1") echo "selected"; ?>>显示</option>
                      <option value="0" <?php if($row->top_navi=="0") echo "selected"; ?>>不显示</option>
                                                            </select></td>
                    </tr>
                  <tr>
                    <td>左侧导航：</td>
                    <td><select name="left_navi" id="left_navi">
                      <option value="1" <?php if($row->left_navi=="1") echo "selected"; ?>>显示</option>
                      <option value="0" <?php if($row->left_navi=="0") echo "selected"; ?>>不显示</option>
                    </select></td>
                    </tr>
                  <tr>
                    <td>首页栏目：</td>
                    <td><select name="index_block" id="index_block">
                      <option value="1" <?php if($row->index_block=="1") echo "selected"; ?>>显示</option>
                      <option value="0" <?php if($row->index_block=="0") echo "selected"; ?>>不显示</option>
                    </select></td>
                  </tr>
                  <tr>
                    ...
                    <td>
                      <select onChange="javascript:expand(document.all.select_pic, document.all.input_pic)" name="select_or_input">
                        <option value="input">上传新的图片</option>
                        <option value="select" selected>选择已有图片</option>
                      </select></td>
                    </tr>
                  <tr>
                    <td>
					...
					<table width="100%"  border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td><table border="0" align="left" cellpadding="10" cellspacing="0" bgcolor="#E6E6E6">
                              <tr>
                                <td align="center" valign="middle"><div align="center"><a href="cut_pic.php?pic_url=<?=$row->picture?>&iwidth=<?=$cfg_colsimg_width?>&iheight=<?=$cfg_colsimg_height?>&cut_self" target="_blank"><img src="<?=$row->picture?>" alt="点击手动生成缩略图"  name="picview" border="0" id="picview" onload="reload_pic('picview');"></a></div></td>
                              </tr>
                          </table></td>
                        </tr>
                      </table>				  
                      <table width="100%"  border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          <td><table width="300" border="0" cellspacing="0" cellpadding="10">
                            <tr align="center">
                              <td><input name="submit_class" type="submit" id="submit_class" value="提 交" onClick="set_upper();"></td>
