...
<HTML><HEAD><TITLE><?=$_REQUEST['part']?>-物理知识系统-<?=$cfg_site_name?></TITLE>
...
href="ask_class.php">问题分类</FONT></A>&nbsp;&gt;&nbsp;<?=$_REQUEST['part']?>
</DIV>
...
border=0></A></DIV>
      <DIV class=fl id=fl_tt><B><?=$_REQUEST['part']?></B></DIV>
      <DIV class=cb></DIV>
    </DIV>
    <DIV id=fl_xx>
      <?php 
	   $query="select * from ".$table_suffix."chapter where part_name='{$_REQUEST['part']}' order by id asc";
	   $result=mysql_query($query);
	   $i=1;
	   while($row=mysql_fetch_object($result)) {
	   ?>
	  <P class=fl_p>·<A title=<?=$row->chapter_name?>  class=f14 href="ask_section.php?chapter=<?=urlencode($row->chapter_name)?>&part=<?=urlencode($_REQUEST['part'])?>"><?=msubstr($row->chapter_name,0,20)?></A></P>   
      <?php
    ...
    <?php 
	  ...
	  $query="select * from ".$table_suffix."ask where part='{$_REQUEST['part']}' and finished='0' and hide='0' order by id desc";
	  else if($_REQUEST['list_for']=="fed")
	  $query="select * from ".$table_suffix."ask where part='{$_REQUEST['part']}' and finished='1' and hide='0' order by id desc";
	  else if($_REQUEST['list_for']=="rec")
	  $query="select * from ".$table_suffix."ask where part='{$_REQUEST['part']}' and hide='0' order by top_time desc";
	?>
    <DIV class="f14 xy1" align=center>
      <?php if((!isset($_REQUEST['list_for']))||($_REQUEST['list_for']=="unf")) { ?>
	  <P class=fl_on id=flk1 >待解决问题</P>
      <P class=fl_of id=flk5 ><A href=?part=<?=urlencode($_REQUEST['part'])?>&list_for=fed>已解决问题</A></P>
      <P class=fl_of id=flk6 ><A href=?part=<?=urlencode($_REQUEST['part'])?>&list_for=rec>推荐问题</A></P>
	  <?php } 
	   elseif($_REQUEST['list_for']=="fed") { ?>
	  <P class=fl_of id=flk1 ><A href=?part=<?=urlencode($_REQUEST['part'])?>&list_for=unf>待解决问题</A></P>
