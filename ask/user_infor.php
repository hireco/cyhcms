<?php 
  ...
  $query="select * from ".$table_suffix."member where id='{$_REQUEST['user_id']}'";
  $result=mysql_query($query); 
  if(!mysql_num_rows($result))  
    { if(isset($_SESSION['user_name']))   require_once("user.php");
	  else require_once("login.php"); 
     }
  else 
  { $row=mysql_fetch_object($result);
    if($_SESSION['user_id']==$row->id) { $who="我"; require_once("user.php"); }
	else {
    $who="他";
	$idkey=md5($row->user_name);
?>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<HTML><HEAD><TITLE><?=$cfg_site_name?> 知识系统-用户信息浏览</TITLE>
...
<DIV class=fr style="PADDING-RIGHT: 5px; PADDING-LEFT: 5px; Z-INDEX: 11; PADDING-BOTTOM: 5px; PADDING-TOP: 5px; POSITION: relative">
<FONT color=#000000>您好，</FONT><A 
href="user.php"><?=$_SESSION['user_name']?></A><SPAN 
...
<?php } else { ?>
<DIV class=fr style="PADDING-RIGHT: 5px; PADDING-LEFT: 5px; Z-INDEX: 11; PADDING-BOTTOM: 5px; PADDING-TOP: 5px; POSITION: relative">
<A href="login.php?to_go=<?php   if(ereg("IIS",$_SERVER['SERVER_SOFTWARE']))  echo urlencode($_SERVER['HTTP_X_REWRITE_URL']);
	  else if(ereg("Apache",$_SERVER['SERVER_SOFTWARE'])) echo urlencode($_SERVER['REQUEST_URI']);
     ?>">登录</A><SPAN class=c9 style="MARGIN: 10px">|</SPAN> <A href="../register.php">注册</A><SPAN class=c9 style="MARGIN: 10px">|</SPAN> 
<A title=知识人首页 href="./" target=_blank>知识人首页</A></DIV>
<?php } ?>
...
&gt; <A title=个人中心 
href="user_infor.php">个人中心</A> &gt; <?=isset($_REQUEST['action'])?$center_pos[$_REQUEST['action']]:"个人首页"?></DIV>
<TABLE id=content cellSpacing=0 cellPadding=0 border=0>
  ...
  <TR>
    <TD id=cont_nav vAlign=top>
      ...
      href="../user_infor.php?host_id=<?=$row->id?>&idkey=<?=$idkey?>" target="_blank"> 
      他的博客</A></H3>
      </DIV>
      ...
      <UL>
        <LI id=li_index><A 
        href="?user_id=<?=$_REQUEST['user_id']?>"><?=$who?>的首页</A>
        <LI id=li_ques><A 
        href="?action=list_question&user_id=<?=$_REQUEST['user_id']?>"><?=$who?>的提问</A> 
        <LI id=li_ans><A 
        href="?action=list_answer&user_id=<?=$_REQUEST['user_id']?>"><?=$who?>的回答</A>
		<LI id=li_msg><A 
        href="?action=msg_list&user_id=<?=$_REQUEST['user_id']?>"><?=$who?>的留言</A> 
        <LI style="BORDER-BOTTOM: medium none"><A 
        href="../user_infor.php?view=profile&host_id=<?=$row->id?>&idkey=<?=$idkey?>"><?=$who?>的资料</A>        </LI>
      </UL></DIV>
	...
	<?php showcfm("您确定要执行此操作吗？","act_cfm");?>
      <DIV class=cont>
      <?php 
	    if(!isset($_REQUEST['action']))  require_once("include/user_infor_index.php");
     ...
     <?php if(!isset($_REQUEST['action'])) { ?>
      ...
      <?php 
	   $query="select content from ".$table_suffix."member_msg where user_name='{$row->user_name}' order by id desc limit 0, 1";
      ...
      <table width="100%"  border="0" cellspacing="0" cellpadding="3">
                       <?php  
					   $width=32; 
					   $query="select * from ".$table_suffix."visitor_list  where visited_id={$row->id}  group by visitor_id order by visit_time desc limit 0, 10";
					   $result=mysql_query($query);
					  ...
					  ?>
                          <tr>
                            <td width="34"><table  border="0" cellpadding="1" cellspacing="1" bgcolor="#CCCCCC">
                                <tr>
                                  <td bgcolor="#FEFEFE">
								  <a href="../user_infor.php?host_id=<?=$friend_list[$i]?>&idkey=<?=$idkey?>" target="_blank" 
							...
							href="../user_infor.php?host_id=<?=$friend_list[$i]?>&idkey=<?=$idkey?>">
                            <?=$row->nick_name?>
                            </a></td>
                          </tr>
						  <tr height="10">
