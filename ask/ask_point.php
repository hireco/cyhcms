...
&nbsp;&gt;&nbsp;<A href="ask_section.php?part=<?=urlencode($_REQUEST['part'])?>&chapter=<?=urlencode($_REQUEST['chapter'])?>"><?=$_REQUEST['chapter']?></A>&nbsp;&gt;&nbsp;<?=$_REQUEST['section']?></DIV>
...
border=0></A></DIV>
    ...
    <DIV id=fl_xx>
      <?php 
	   $query="select * from  ".$table_suffix."section where section_name='{$_REQUEST['section']}'";
	   $result=mysql_query($query);
	   $point=mysql_result($result,0,"point");
	   $section_name=mysql_result($result,0,"section_name");
	   $point=ereg_replace("，",",",$point);
	   $point=explode(",",$point);
	   $i=1;
	   while($point[$i-1]) {
	   ?>
	  <P class=fl_p>·<A title=<?=$point[$i-1]?> class=f14 href="ask_list.php?point=<?=urlencode($point[$i-1])?>&section=<?=urlencode($section_name)?>&chapter=<?=urlencode($_REQUEST['chapter'])?>&part=<?=urlencode($_REQUEST['part'])?>"><?=msubstr($point[$i-1],0,20)?></A></P>   
      <?php
	   if($i%4==0) echo "<P class=cb></P>"; 
	   $i++;
	  } 
	  ?>
	  <P class=cb></P>  
    ...
    <?php 
	  ...
	  $query="select * from ".$table_suffix."ask where part='{$_REQUEST['part']}' and chapter='{$_REQUEST['chapter']}' and  section='{$_REQUEST['section']}' and finished='0' and hide='0' order by id desc";
	  else if($_REQUEST['list_for']=="fed")
	  $query="select * from ".$table_suffix."ask where part='{$_REQUEST['part']}' and chapter='{$_REQUEST['chapter']}' and  section='{$_REQUEST['section']}' and finished='1' and hide='0' order by id desc";
	  else if($_REQUEST['list_for']=="rec")
	  $query="select * from ".$table_suffix."ask where part='{$_REQUEST['part']}' and chapter='{$_REQUEST['chapter']}' and  section='{$_REQUEST['section']}' and hide='0' order by top_time desc";
	?>
    <DIV class="f14 xy1" align=center>
      <?php if((!isset($_REQUEST['list_for']))||($_REQUEST['list_for']=="unf")) { ?>
	  <P class=fl_on id=flk1 >待解决问题</P>
      <P class=fl_of id=flk5 ><A href=?part=<?=urlencode($_REQUEST['part'])?>&chapter=<?=urlencode($_REQUEST['chapter'])?>&section=<?=urlencode($_REQUEST['section'])?>&list_for=fed>已解决问题</A></P>
      <P class=fl_of id=flk6 ><A href=?part=<?=urlencode($_REQUEST['part'])?>&chapter=<?=urlencode($_REQUEST['chapter'])?>&section=<?=urlencode($_REQUEST['section'])?>&list_for=rec>推荐问题</A></P>
	  <?php } 
	   elseif($_REQUEST['list_for']=="fed") { ?>
	  <P class=fl_of id=flk1 ><A href=?part=<?=urlencode($_REQUEST['part'])?>&chapter=<?=urlencode($_REQUEST['chapter'])?>&section=<?=urlencode($_REQUEST['section'])?>&list_for=unf>待解决问题</A></P>
