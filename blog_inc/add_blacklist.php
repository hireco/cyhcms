...
<?php 
  if(!isset($_SESSION['user_name']))   ShowMsg("您还没有登录,请先登录再操作!",-1);
    ...
    if(isset($_REQUEST['host_id'])) {
	 ...
	 elseif($_REQUEST['host_id']==$_SESSION['user_id'])  ShowMsg("不能加自己的黑名单哦!",-1); 
	 else {
	  ...
	  if(md5($host_name)<>$_REQUEST['idkey'])  ShowMsg("对不起,错误的访问",-1); 
	   ...
	   if($black_list) {
		...
		if(in_array($_REQUEST['host_id'],$friend_list))  { ShowMsg("请先将".$host_nick."从您的好友名单中去掉",-1); exit;}
		if(in_array($_REQUEST['host_id'],$black_list))  { ShowMsg($host_nick."已经在黑名单中",-1); exit;}
		else { 
		$black_list=implode(",",$black_list);
		$black_list=$black_list.",".$_REQUEST['host_id']; 
	    $black_list=explode(",",$black_list);
	    $black_list=array_unique($black_list);
	    $black_list=implode(",",$black_list);
          } 
		}
	   else  $black_list=$_REQUEST['host_id'];
	   
	   $result=mysql_query("update  ".$table_suffix."member set black_list='$black_list' where user_name='{$_SESSION['user_name']}'");
       if($result)  ShowMsg("恭喜,成功加".$host_nick."致黑名单中!",-1); 
