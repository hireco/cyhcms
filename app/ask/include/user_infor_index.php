<DIV>
	...
	?>
     <table  border="0" align="center" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC">
	   ...
	   <a href="../user_infor.php?view=profile&host_id=<?=$row->id?>&idkey=<?=$idkey?>"><img src="<?=$sample_pic?>"  alt="了解主人" border="0"  >	   </a></div></td>
       </tr>
       </table>
	  <br>
	  <A href="../blog_inc/add_friend.php?host_id=<?=$row->id?>&idkey=<?=$idkey?>" target="_blank" class=c07f>加为好友</A> <A href="../user_infor.php?view=liuyan&host_id=<?=$row->id?>&idkey=<?=$idkey?>#say" target="_blank" class=c07f>博客留言</A> </DIV>
      <DIV class=user_detail>
      <H3><?=$row->nick_name?></H3>
      <DIV class=user_fg>他的编号：<?=$row->id?><BR>
	  性&nbsp;&nbsp;&nbsp;&nbsp;别：<?=$row->sex=="m"?"<font color=blue>男</font>":"<font color=red>女</font>";?> <BR>
      <DIV id=div_birth>出生日期：<?=$row->birthday?><BR>
	  </DIV>
