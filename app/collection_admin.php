...
<?php  require_once("center_header.php"); 
   ...
   $result=mysql_query("delete from  ".$table_suffix."member_url  where id={$_REQUEST['id']} and user_name='{$_SESSION['user_name']}'");
   if($result) {
  ...
  <tr>
     ...
     <tr><td width="10" bgcolor="#D0D2E3"></td>
		    ...
		    { 
		      echo "<td bgcolor=\"#D0D2E3\"><table  border=\"0\" cellspacing=\"0\" cellpadding=\"2\"><tr>"; 
			   for($i=0;$i<=$num_of_folder;$i++){  
				...
				if($i==0) {
				 echo "<td"; 
			     if(!isset($_REQUEST['folder_name']))  echo " bgcolor=\"#FFFFFF\">所有收藏夹</td>";   
			     else  echo "><a href=\"collection_admin.php?more_folder\" style=\"text-decoration:underline \" >所有收藏夹</a></td>";
			         }
				else { 
				 ...
				 else echo "<a href=\"?folder_name=".urlencode($folder_row->folder_name)."&more_folder\" style=\"text-decoration:underline \" >".$folder_row->folder_name."</a>";
				 echo "</div></td>";          
                 }
			    }
			   echo "</tr></table></td>";
               ...
               echo "<td"; 
			   if(!isset($_REQUEST['folder_name']))  echo " bgcolor=\"#FFFFFF\">所有收藏夹</td>";   
			   else  echo "><a href=\"collection_admin.php\" style=\"text-decoration:underline \" >所有收藏夹</a></td>";
			   $i=1;
			   ...
			   echo "><div align=\"center\">"; 
			   if($folder_row->folder_name==$_REQUEST['folder_name'])   echo $folder_row->folder_name;
			   else echo "<a href=\"?folder_name=".urlencode($folder_row->folder_name)."\" style=\"text-decoration:underline \" >".$folder_row->folder_name."</a>";
			   echo "</div></td>";
			   $i++;
			   }
			   echo "<td><a href=\"?more_folder\" style=\"text-decoration:underline \">更多></a></td>"; 
			   echo "<td><a href=\"collection.php?action=add\" style=\"text-decoration:underline \">添加</a></td>";
			   echo "</tr></table></td>";
		    }	 
		 ?>
	  </tr>
      <tr>
		...
		<?php
		  ...
		  $query="select * from ".$table_suffix."member_url  where ".$condition." and user_name='{$_SESSION['user_name']}'  order by post_time desc";
   ...
   ?>
            <table width="100%"  border="0" cellspacing="0" cellpadding="0">
              ...
              <tr>
                <td>&nbsp;</td>
                <td><a href="<?=$row->content?>" target=_blank style="text-decoration:underline ">
                <?=$row->infor_title?>
                </a></td>
                <td class="fonts"><?=substr($row->post_time,3,11)?>
                  </td>
                <td><a href="collection_admin.php?folder_name=<?=urlencode($row->folder_name)?>">
                  <?=$row->folder_name?>
                </a></td>
                <td><a href="collection.php?action=edit&id=<?=$row->id?>">编 辑</a></td>
                <td>
                  <div align="center">
                    <?php 
				    echo "<input type=button value=\"删除\" class=\"INPUT\" onClick=\" if(really())  no_show('?id=".$row->id."&action=delete')\" target=\"_self\" >";  ?>
                  </div></td>
              </tr>
              <?php } 
			     }
